...
def demo(runner, args):
    with gr.Blocks(title="YOLO-World") as demo:
        ...
        with gr.Tab("Video"):
            with gr.Row():
                ...
                with gr.Column(scale=0.5):
                    # out_video_image = gr.Image(type='pil', label='Processing Image')
        ...
        with gr.Row():
            input_text = gr.Textbox(label='Enter the classes to be detected, separated by comma',
                                    value='person, face',
                                    elem_id='textbox')

        video_submit.click(partial(run_inference, runner),
                                [input_video_file, input_text, max_num_boxes, score_thr, nms_thr],
                                [output_video])
        video_clear.click(fn=video_clear_fn, outputs=[input_video_file, input_text, out_video_image, output_video])
        image_submit.click(partial(run_inference, runner),
                                [input_image_file, input_text, max_num_boxes, score_thr, nms_thr],
                                [output_image])
        image_clear.click(lambda: [[], '', ''], None,
                                [input_image_file, input_text, output_image])
